...
import { getMyRecipes } from "../utils/api";
import {
  Mail, BookOpen, LogOut, Edit3, Calendar,
...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    getMyRecipes()
      .then((recipes) => {
        setMyRecipeCount(recipes.length);
        setMyRecentRecipes(recipes.slice(0, 4));
      })
      .catch(console.error)
    ...
    <div className="max-w-6xl mx-auto px-6 py-10 mt-8">
      <div className="bg-white shadow-lg rounded-2xl p-8 relative overflow-hidden">
        ...
        <div className="relative flex flex-col items-center text-center z-10">
          ...
          {menuOpen && (
            <div className="absolute top-16 right-6 bg-white border border-gray-200 shadow-lg rounded-xl p-3 flex flex-col gap-2 w-48 z-20">
              <button
      ...
      {/* Recent Recipes */}
      <div className="mt-12">
        <h3 className="text-2xl font-semibold text-gray-800 mb-6">My Recent Recipes</h3>
        {loading ? (
          <p className="text-gray-500">Loading recipes...</p>
        ) : myRecentRecipes.length === 0 ? (
          <p className="text-gray-500">
            You haven't added any recipes yet.{" "}
            <Link to="/add" className="text-red-500 underline">Create your first one!</Link>
          </p>
        ) : (
      ...
      {/* Blog placeholder */}
      <div className="mt-16">
        <h3 className="text-2xl font-semibold text-gray-800 mb-6">My Blog Posts</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="bg-white rounded-xl shadow-md hover:shadow-lg transition transform hover:-translate-y-1 p-5">
              <div className="flex items-center justify-between mb-3">
                <FileText className="w-5 h-5 text-red-500" />
                <span className="text-xs text-gray-400">Oct {20 + i}, 2025</span>
              </div>
              <h4 className="font-semibold text-gray-800 mb-2">How to Cook Like a Pro #{i}</h4>
              <p className="text-gray-500 text-sm line-clamp-3">
                Discover essential kitchen skills, tricks, and time-saving tips to improve your cooking.
              </p>
              <Link to="#" className="text-red-500 text-sm font-medium mt-3 inline-flex items-center">
