...
export default function RecipeDetail() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (user) {
          ...
          setIsOwner(String(ownerId) === String(user._id));

          const favIds = await getFavoriteIds();
          setIsFavorited(favIds.map((f) => f.toString()).includes(data._id.toString()));
        }
      } catch (err) {
        console.error(err);
  ...
  const handleFavorite = async () => {
    ...
    try {
      const updatedIds = await toggleFavorite(recipe._id);
      setIsFavorited(updatedIds.map((f) => f.toString()).includes(recipe._id.toString()));
    } catch (err) {
  ...
  const image = recipe.image || "https://via.placeholder.com/1200x800";
  const authorName = recipe.userId?.name || "Unknown";
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <main className="lg:col-span-2 space-y-6">
          ...
          <div className="flex items-center gap-4 text-gray-700">
            ...
            >
              <Bookmark
                className={`w-5 h-5 ${isFavorited ? "fill-amber-600 text-amber-600" : ""}`}
              />
          ...
          <section className="bg-white rounded-lg shadow p-6 space-y-6">
            ...
            {(recipe.steps || []).map((step, idx) => (
              <div key={idx} className="flex gap-3">
                <div className="w-10 h-10 flex-shrink-0 flex items-center justify-center bg-amber-100 text-amber-700 rounded-full font-bold">
                  {idx + 1}
                </div>
                <p className="text-gray-800 pt-2">{step}</p>
              </div>
            ))}
          </section>
